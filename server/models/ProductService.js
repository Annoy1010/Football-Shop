...
function getProductByGrassId(req, res){
    ...
    db.query(`SELECT * FROM SHOES s, SHOES_IMAGE si, SHOES_SIZE ss, trademark tm WHERE s.shoesId = si.shoesId and s.shoesId = ss.shoesId and s.trademarkId = tm.trademarkId and s.grassId = '${grassid}'`,
        (err, result) => {
            if (err) {
                throw err;
            } else {
...
function getProductByPositionName(req, res){
    ...
    db.query(`SELECT * FROM SHOES s, SHOES_IMAGE si, SHOES_SIZE ss, PLAY_POSITION pp WHERE s.shoesId = si.shoesId and s.shoesId = ss.shoesId and s.playPositionId = pp.playpositionId and pp.playPositionName = '${positionName}'`,
        (err, result) => {
            if (err) {
                throw err;
            } else {
